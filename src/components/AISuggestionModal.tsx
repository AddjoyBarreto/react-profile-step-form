    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" role="dialog" aria-modal="true">
      <div className="bg-white w-full max-w-2xl rounded-xl shadow-xl p-5" tabIndex={-1} ref={dialogRef}>
        <div className="flex justify-between items-center mb-3">
          <h3 className="text-lg font-semibold">{t('aiSuggestion')}</h3>
          <button aria-label={t('close')} onClick={onClose} className="p-2 rounded hover:bg-gray-100">✕</button>
        </div>
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 rounded-lg bg-gray-100 text-gray-800 hover:bg-gray-200">{t('discard')}</button>
          <button onClick={() => onAccept(text)} className="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700">{t('accept')}</button>
        </div>
      </div>
    </div>
  );
